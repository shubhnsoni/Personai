...
function screen() {
  ...
  document.querySelector('#screen-name').textContent = `${fullscreenChoice.name} · Icon ${iconOption} · Actual page-loading size`;
...
document.querySelector('#save-option').addEventListener('click',async()=>{
  ...
  if(!selected){status.textContent='Choose one of the six loader designs first.';return;}
  button.disabled=true;
  try{
    const response=await fetch('/api/loader-options',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({requestId:crypto.randomUUID(),design:selected,iconOption,name:document.querySelector('#preset-name').value})});
