...
/**
 * Ou le bot porte son regard quand quelque chose d'exterieur le pilote ΓÇö le
 ...
 *   yeux suivait celle de chaque expression ΓÇö ┬½ neutre ┬╗ regarde a +28,6deg quand
 *   les autres sont entre -9 et +9 ΓÇö donc les yeux tombaient d'un coup au premier
 ...
 * confondent pas : quand le pointeur bouge, la derive doit s'eteindre ΓÇö cumulees,
 ...
 * derriere la boule et revenir de l'autre cote ΓÇö et `-360deg` etant le meme
...
export class BotEngine {
   ...
   * son inertie ΓÇö le regard n'atteint jamais tout a fait un curseur qui bouge.
   */
   ...
   * chaque rattrapage ΓÇö le suivi tremblerait au lieu de glisser.
   *
  ...
  setLook(look: Look | null, now: number, morph = BotEngine.LOOK_MORPH) {
     ...
     * reposerait plus jamais. C'est arrive pour de vrai ΓÇö un
   ...
   * ΓÇö c'est la meme cause, donc ce doit etre le meme mouvement.
   *
  ...
  private decalageAtTime(now: number, state: StateId): { x: number; y: number } {
     ...
     * courbe. Jamais sur la valeur interpolee ΓÇö celle-la n'a pas d'identite et n'existe
   ...
   * et l'export GIF s'ouvrait sur une boule sans yeux ΓÇö la comete a un `eyeAlpha` nul.
   *
   ...
   * a son propre temps ecoule ΓÇö donc encore en train de s'animer, ce qui est voulu.
   */
  private origine(
    now: number,
    shape: number[] | null,
   ...
   * l'etat qu'on quitte pendant tout le fondu ΓÇö le ┬½ ! ┬╗ d'`alert` se figerait en pleine
   * course ΓÇö alors qu'il n'y a rien a corriger hors morph : l'etat quitte y est deja
  ...
  sample(now: number): BotFrame {
    ...
    // rejouable ΓÇö relire une date d'avant la fin du fondu ne le retrouverait
    ...
    if (origine) {
      ...
      // donnerait un ratio negatif, que l'ease-out extrapole ΓÇö la silhouette
      ...
      // de l'etat quitte, que `setState` renseigne toujours en meme temps que l'origine ΓÇö
    ...
    const gaze = {
      ...
      // melange, sinon la cible l'annulerait en meme temps que la pose ΓÇö or elle
