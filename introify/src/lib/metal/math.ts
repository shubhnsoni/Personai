...
const IST_MONTHS = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"] as const

/** Same string on server and client. Asia/Kolkata is UTC+5:30 with no DST. */
export function formatIstClock(iso?: string | null): string | null {
    if (!iso) return null
    ...
    hour = hour % 12 || 12
    return `${hour}:${minute} ${ampm}, ${day} ${month}`
}
