...
export function buildSystemPrompt(profile: ProfileWithRelations, contextDocs: ProfileDocument[], currency: DisplayCurrency = "USD"): string {
    ...
    if (showServices && profile.serviceOfferings && profile.serviceOfferings.length > 0) {
        ...
        if (takeBook) {
            ...
            const recruit = kitSvc === "RECRUITMENT_AGENCY"
            ...
            const preferLine = events
                ...
                : realEstate
                  ...
                  : recruit
                    ? `Prefer Book / ${href} for enquire, hire, and schedule-interview; WhatsApp is secondary. Never say session, treatment, or appointment.`
    ...
    if (showShop && profile.digitalProducts && profile.digitalProducts.length > 0) {
        ...
        productsSection = restaurant
            ...
            : role === "JEWELRY_RETAIL"
                ? `\n## Jewellery\nPrices are metal (weight × today's city purity rate) plus making. Never invent a rate or a gram weight. When they ask bridal, mangalsutra, gold rate, or catalogue: cite listed SKUs and/or City Rates and deep-link ${profile.slug ? jewelryMenuPath(profile.slug) : "/menu"}. WhatsApp / walk-in secondary — never phone-only. If /book is empty, do not invent visit booking.\n${productList}\n${jewelryMenuPromptGuidance({ slug: profile.slug, role: profile.roleTemplate, goal: profile.primaryGoal, hasCatalog: true, hasBoard: Boolean(board), hasBookableServices: Boolean(profile.serviceOfferings?.length), whatsapp: profile.whatsapp }).join(" ")}`
    ...
    const about = kitAbout(role, profile.displayName)
    const headline = profile.headline?.trim() || about.headline
    const bio = profile.bio?.trim() || about.bio
    const identity = cloneOperatingPrompt({
...
## About ${profile.displayName} (${roleDescription})
