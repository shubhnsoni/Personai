...
function kit(role?: string | null, extras?: SurfaceExtras | null) {
    const base = KIT[resolveKitRole(role) || ""] || KIT.CUSTOM
...
export function shopNavLabel(role?: string | null) {
    const kitRole = resolveKitRole(role)
    if (kitRole === "RESTAURANT") return "Menu"
    if (kitRole === "JEWELRY_RETAIL") return "Jewellery"
    if (kitRole === "JEWELRY_WHOLESALE") return "Stock"
    if (kitRole === "DISTRIBUTOR") return "Inventory"
    if (kitRole === "PHARMACY") return "Medicines"
    if (kitRole === "AUTO_PARTS") return "Parts"
    return "Shop"
}

export function leadsNavLabel(role?: string | null) {
    const kitRole = resolveKitRole(role)
    if (kitRole === "JEWELRY_WHOLESALE") return "Parties"
    if (kitRole === "DISTRIBUTOR") return "Dealers"
    return "Leads"
}

export function salesNavLabel(role?: string | null) {
    const kitRole = resolveKitRole(role)
    if (kitRole === "JEWELRY_RETAIL" || kitRole === "JEWELRY_WHOLESALE") return "Cashflow"
    if (kitRole === "DISTRIBUTOR") return "Orders"
    return "Sales"
}
...
export function defaultFulfillment(role?: string | null, extras?: SurfaceExtras | null): "PHYSICAL" | "DIGITAL" | "BOTH" {
    ...
    const kitRole = resolveKitRole(role)
    if (kitRole === "RESTAURANT" || kitRole === "SHOP" || kitRole === "JEWELRY_RETAIL" || kitRole === "JEWELRY_WHOLESALE" || kitRole === "DISTRIBUTOR" || kitRole === "PHARMACY" || kitRole === "AUTO_PARTS") return "PHYSICAL"
    return "PHYSICAL"
}

export function calendarNoun(role?: string | null) {
    const kitRole = resolveKitRole(role)
    if (kitRole === "RESTAURANT") return "Reservations"
    if (kitRole === "CONSULTANT" || kitRole === "CA" || kitRole === "COACH") return "Sessions"
    return "Bookings"
}
...
export function isDigitalCatalogItem(product: { fulfillment?: string | null; type?: string | null }) {
    if (product.fulfillment === "PHYSICAL") return false
    if (product.fulfillment === "DIGITAL" || product.fulfillment === "BOTH") return true
    return product.type === "PDF" || product.type === "VIDEO" || product.type === "AUDIO"
}
...
export function publicChipAllowed(role: string | null | undefined, chip: string, extras?: SurfaceExtras | null) {
    ...
    switch (chip) {
        case "shop":
            return shop
        case "products":
            return shop && fieldOn(role, "shopDigital", extras)
        case "book":
        case "services":
        case "rates":
            return book
        ...
        case "cases":
        case "history":
            return fieldOn(role, "portfolio", extras) || resolveKitRole(role) === "CUSTOM"
        case "guide":
            return fieldOn(role, "shopDigital", extras)
        case "wa":
            return fieldOn(role, "whatsappUpi", extras) || shop || hasSurface(role, "calendar", extras)
        case "tip":
            return fieldOn(role, "whatsappUpi", extras) || resolveKitRole(role) === "CREATOR" || resolveKitRole(role) === "CUSTOM"
        default:
            return true
    }
}
