...
async function writeBoard(profileId: string, fallbackCity: string, fallbackSlug: string) {
    ...
    await prisma.profile.update({
        where: { id: profileId },
        data: {
            personalityConfig: writeGoldBoard(row?.personalityConfig, existing
                ? { ...existing }
                : {
                    city: fallbackCity,
                    ...
                    source: "city-feed",
                    k24PaisePer10g: DEFAULT_RATES.k24PaisePer10g,
                    k22PaisePer10g: DEFAULT_RATES.k22PaisePer10g,
                    k18PaisePer10g: DEFAULT_RATES.k18PaisePer10g,
...
export async function seedRole(profileId: string, role: string) {
    ...
    if (role === "JEWELRY_RETAIL") {
        await prisma.profile.update({ where: { id: profileId }, data: { liveChatEnabled: true } })
    ...
    if (role === "PHARMACY") {
        ...
        if (existing === 0) {
            ...
            await prisma.digitalProduct.updateMany({
                where: { profileId, OR: [{ fulfillment: { not: "PHYSICAL" } }, { type: { not: "PHYSICAL" } }] },
        ...
        const row = await prisma.profile.findUnique({
            where: { id: profileId },
            select: { displayName: true, headline: true, bio: true, welcomeMessageOverride: true },
        })
        if (row && (!row.bio?.trim() || !row.headline?.trim())) {
            const about = kitAbout("PHARMACY", row.displayName)
            await prisma.profile.update({
                where: { id: profileId },
                data: {
                    headline: row.headline?.trim() || about.headline,
                    bio: row.bio?.trim() || about.bio,
                    ...(!row.welcomeMessageOverride?.trim()
    ...
    if (role === "SHOP") {
        ...
        if (existing === 0) {
            await prisma.digitalProduct.createMany({
                data: [
                    { profileId, title: "House favourite", category: "Everyday", type: "PHYSICAL", fulfillment: "PHYSICAL", currency: "INR", priceCents: 14900, stock: 12, isActive: true },
                    { profileId, title: "Counter pack", category: "Everyday", type: "PHYSICAL", fulfillment: "PHYSICAL", currency: "INR", priceCents: 9900, stock: 20, isActive: true },
                ],
            })
        }
    }
