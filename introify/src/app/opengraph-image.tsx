...
export const alt = "Introify — Your business, one page, ready to grow. Answers, bookings and a catalog in one link."
    ...
    return new ImageResponse(
        <div style={{ width: "100%", height: "100%", display: "flex", position: "relative", background: "#073d30", color: "#f5f6e9", fontFamily: "sans-serif", padding: "50px 64px" }}>
            <div style={{ display: "flex", flexDirection: "column", width: 780 }}>
                ...
                <div style={{ display: "flex", marginTop: 28, fontSize: 22, lineHeight: 1.5, color: "#c4d5b9", maxWidth: 570 }}>Answers, bookings and a catalog. One page that does the work.</div>
            ...
            <div style={{ display: "flex", position: "absolute", right: 65, top: 110, width: 264, height: 390, background: "#f4f4e8", color: "#173f2d", borderRadius: 18, padding: "24px 25px", transform: "rotate(8deg)", flexDirection: "column", boxShadow: "0 18px 45px rgba(0,30,20,.25)" }}>
                <div style={{ display: "flex", fontSize: 11, letterSpacing: 2, color: "#5c7650" }}>ONE LINK. A PAGE THAT WORKS.</div>
                <div style={{ display: "flex", marginTop: 24, fontSize: 35, lineHeight: 1.05, fontWeight: 700, letterSpacing: -1.5 }}>Ask. Book. Buy.</div>
                {["Your page", "Your assistant", "Let’s connect"].map((label, index) => <div key={label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", marginTop: index === 0 ? 25 : 8, borderRadius: 7, background: index === 2 ? "#a3db42" : "#e4eacf", fontSize: 13 }}><span>{label}</span><span>↗</span></div>)}
            </div>
