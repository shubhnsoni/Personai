...
function productWrite(data: ProductData) {
    ...
    return {
        ...
        variantsJson: (() => {
            ...
            const pdp = data.pdp !== undefined ? data.pdp : parsePdpDisplay(data.existingVariantsJson)
            json = writePdpDisplay(json ?? data.existingVariantsJson, pdp)
...
export async function confirmProductOrder(purchaseId: string) {
    ...
    const paid = await prisma.productPurchase.findUnique({
        where: { id: purchaseId },
        include: { product: { select: { profileId: true, priceCents: true, currency: true } } },
    }).catch(() => null)
    if (paid?.status === "COMPLETED") {
        const { recordMoneyEvent } = await import("@/lib/admin/money")
        await recordMoneyEvent({
            profileId: paid.product.profileId,
            kind: "PURCHASE",
            subjectId: paid.id,
            amountCents: paid.product.priceCents,
            currency: paid.product.currency,
            payMethod: paid.payMethod || "UPI",
            payStatus: "PAID",
            visitorEmail: paid.visitorEmail,
        })
    }
    revalidatePath("/dashboard/orders")
    revalidatePath("/dashboard/money")
