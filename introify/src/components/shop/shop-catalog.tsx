    ...
    return (
        <div className="space-y-4">
            {items.some((p) => p.ar) ? (
                ...
                >
                    <span>
                        <span className="block text-sm font-medium text-cyan-800 dark:text-cyan-100">View in your space</span>
                        <span className="mt-0.5 block text-[12px] text-cyan-700/80 dark:text-cyan-200/70">Open the camera, swipe products, save a photo</span>
                    </span>
                    <span className="rounded-full bg-cyan-400 px-3 py-1.5 text-xs font-medium text-zinc-950">See it in 3D</span>
                </Link>
            ) : null}
            {(whatsapp || upiId) && (
                <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-border bg-card px-3 py-2.5 text-sm">
                    {upiId ? <span className="text-muted-foreground">UPI <span className="text-foreground">{upiId}</span></span> : null}
                    {wa ? (
                ...
                <div className="flex gap-1 overflow-x-auto">
                    {["all", ...makes].map((m) => (
                        <button
                            ...
                            type="button"
                            onClick={() => setMake(m)}
                            className={`shrink-0 rounded-full px-3 py-1 text-xs ${make === m ? "bg-foreground text-background" : "bg-muted text-muted-foreground"}`}
                        >
                            {m === "all" ? "All cars" : m}
                        </button>
                    ))}
            ...
            {yearOptions.length > 0 ? (
                <div className="flex items-center gap-2">
                    <label className="shrink-0 text-xs text-muted-foreground" htmlFor="fitment-year">Year</label>
                    <select
                        id="fitment-year"
                        value={year}
                        onChange={(e) => setYear(e.target.value)}
                        className="h-8 rounded-full border border-border bg-muted px-3 text-xs text-foreground"
                    >
                ...
                <div className="flex gap-1 overflow-x-auto">
                        ...
                        <>
                            {(["all", "VEG", "NONVEG"] as const).map((d) => (
                                <button
                                    ...
                                    type="button"
                                    onClick={() => setDiet(d)}
                                    className={`shrink-0 rounded-full px-3 py-1 text-xs ${diet === d ? "bg-foreground text-background" : "bg-muted text-muted-foreground"}`}
                                >
                    ...
                    {["all", ...cats].map((c) => (
                        <button
                            ...
                            type="button"
                            onClick={() => setCat(c)}
                            className={`shrink-0 rounded-full px-3 py-1 text-xs ${cat === c ? "bg-foreground text-background" : "bg-muted text-muted-foreground"}`}
                        >
                ...
                <div className="grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-2.5 lg:grid-cols-4 lg:gap-2">
                    {rows.map((p) => (
                        <Link
                            key={p.id}
                            href={`/${slug}/shop/${p.id}`}
                            className="overflow-hidden rounded-2xl border border-border bg-card lg:rounded-xl"
                        >
                            <ShopCover src={p.thumbnailUrl} type={p.type} title={p.title} className="aspect-square" />
                            <div className="space-y-0.5 p-3 lg:p-2">
                                <p className="flex items-start gap-1.5 line-clamp-2 text-sm font-medium leading-snug text-foreground lg:text-[13px]">
                                    {showDiet && p.diet ? <span className={`mt-1 h-2 w-2 shrink-0 rounded-sm ${dietDotClass(p.diet)}`} title={dietLabel(p.diet) || ""} /> : null}
                                    <span className="min-w-0">{p.title}</span>
                                    {p.rxRequired ? (
                                        <span className="mt-0.5 shrink-0 rounded-full bg-muted px-1.5 py-0.5 text-[10px] font-normal text-muted-foreground">Rx</span>
                                    ) : null}
                                </p>
                                {p.metalLine ? (
                                    <p className="text-[11px] text-muted-foreground">{p.metalLine}</p>
                                ) : p.extraLine ? (
                                    <p className={`text-[11px] ${p.extraWarn ? "text-amber-700 dark:text-amber-400/80" : "text-muted-foreground"}`}>{p.extraLine}</p>
                                ) : null}
