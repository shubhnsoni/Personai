...
function welcomeTopics(profile: ProfileViewProps["profile"]) {
    const role = resolveKitRole(profile.roleTemplate) || profile.roleTemplate
    if (role === "RESTAURANT") {
...
function buildGoalChips(
    ...
    const catalog: Record<string, ChipDef> = {
        ...
        products: {
            ...
            label: "Digital shop",
            available:
                profile.roleTemplate !== "RESTAURANT"
                && (profile.digitalProducts || []).some((p) => isDigitalCatalogItem(p)),
            icon: <DollarSign className="w-3.5 h-3.5" />,
    ...
    const orderByKit: Record<string, string[]> = {
        ...
        PHARMACY: ["shop", "wa", "about"],
        AUTO_PARTS: ["shop", "wa", "about"],
