...
export function BrandLoadingVisual({ compact = false, complete = false }: { compact?: boolean; complete?: boolean }) {
    return (
        <div className={cn("brand-loading-visual", compact && "brand-loading-visual--compact")} data-loading-option="6" data-icon-option="9" data-complete={complete} aria-hidden="true">
            <svg className="brand-loading-circle" viewBox="0 0 128 128" fill="none">
                <circle cx="64" cy="64" r="50" className="brand-loading-circle-track" />
                <circle cx="64" cy="64" r="50" pathLength="100" className="brand-loading-circle-progress" />
            </svg>
            {["light", "dark"].map(mode => <svg key={mode} viewBox="0 0 260 260" className={cn("brand-loading-symbol", mode === "light" ? "dark:hidden [.auth-scene_&]:hidden" : "hidden dark:block [.auth-scene_&]:block")}>
                <image className="brand-loading-symbol-motion" href={`/brand/loaders/main/icon-${mode}.svg`} width="260" height="260" />
                <image className="brand-loading-symbol-still" href={`/brand/loaders/main/icon-${mode}-still.svg`} width="260" height="260" />
            </svg>)}
        </div>
    )
}
