...
import { parsePdpDisplay, type PdpDisplay } from "@/lib/shop/pdp-display"
...
function FieldLabel({
    ...
    return (
        <label className={cn("block min-w-0 space-y-1", className)}>
            <span className="block px-0.5 text-[11px] font-medium text-muted-foreground">{label}</span>
            {children}
        </label>
    )
}
...
export function QuickAddSheet({
    ...
    const [pdpShow, setPdpShow] = useState<PdpDisplay>({
        showMore: false,
        showRelated: false,
    ...
    useEffect(() => {
        ...
        setPdpShow(parsePdpDisplay(product?.variantsJson))
    ...
    function reset() {
        ...
        setFitYearFrom("")
        setFitYearTo("")
        setPdpShow({ showMore: false, showRelated: false, showBestsellers: false })
    }
                    ...
                    onSubmit={async (e) => {
                        ...
                        try {
                            ...
                            const payload = {
                                ...
                                pdp: {
                                    ...pdpShow,
                                    showMore: pdpShow.showMore && photos.length > 1,
                                },
                            }
                ...
                >
                    <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 pt-3">
                        ...
                        <PhotoStage
                            ...
                            hint="Take a photo or pick from the gallery. First photo is the cover."
                            onAdd={async (files) => {
                                setUploading(true)
                                try {
                                    const urls: string[] = []
                        ...
                        <FieldLabel label={restaurant ? "Dish name" : gold ? "Piece name" : pharmacy ? "Medicine name" : autoParts ? "Part name" : "Item name"}>
                            <Input
                                value={title}
                                onChange={(e) => setTitle(e.target.value)}
                                placeholder={restaurant ? "e.g. Butter chicken" : gold ? "e.g. Light bangle" : pharmacy ? "e.g. Paracetamol 650" : autoParts ? "e.g. Front brake pad" : "e.g. House favourite"}
                                autoFocus
                        ...
                        <div className="grid grid-cols-3 gap-2.5">
                            ...
                            <FieldLabel label="Purity">
                                <select
                                    ...
                                    className="h-12 w-full rounded-2xl border border-border/70 bg-background px-3 text-base"
                                >
                        ...
                        <div className={cn("grid gap-2.5", gold || !(showPhysical || showMenu) ? "grid-cols-1" : "grid-cols-2")}>
                            {gold ? null : (
                            ...
                            <FieldLabel label={fulfillment === "DIGITAL" ? "Stock (optional)" : restaurant ? "Portions in stock" : "Stock quantity"}>
                                <Input
                                    type="number"
                                    min="0"
                                    value={stock}
                        ...
                        <label className="flex h-12 items-center justify-between rounded-2xl bg-muted/50 px-3.5 text-sm">
                            More details cards
                            <Switch
                                checked={pdpShow.showMore}
                                disabled={photos.length < 2}
                                onCheckedChange={(on) => setPdpShow((cur) => ({ ...cur, showMore: on }))}
                            />
                        </label>
                        <label className="flex h-12 items-center justify-between rounded-2xl bg-muted/50 px-3.5 text-sm">
                            Same category
                            <Switch
                                checked={pdpShow.showRelated}
                                onCheckedChange={(on) => setPdpShow((cur) => ({ ...cur, showRelated: on }))}
                            />
                        </label>
                        <label className="flex h-12 items-center justify-between rounded-2xl bg-muted/50 px-3.5 text-sm">
                            Best sellers
                            <Switch
                                checked={pdpShow.showBestsellers}
                                onCheckedChange={(on) => setPdpShow((cur) => ({ ...cur, showBestsellers: on }))}
                            />
                        </label>
                        {restaurant ? (
                        ...
                        {more ? (
                            <div className="space-y-3 pb-2">
                                <FieldLabel label={showMenu ? "Menu section" : "Category"}>
                                    <Input value={category} onChange={(e) => setCategory(e.target.value)} placeholder={showMenu ? "Starters, Mains, Snacks" : "e.g. Everyday"} className="h-11 rounded-2xl" />
                                </FieldLabel>
                                {showMenu ? (
                                    <>
                                        <p className="px-0.5 text-[11px] font-medium text-muted-foreground">Diet</p>
                                        ...
                                        <p className="px-0.5 text-[11px] font-medium text-muted-foreground">Served</p>
                                        <div className="grid grid-cols-4 gap-1.5">
                                        ...
                                        <p className="px-0.5 text-[11px] font-medium text-muted-foreground">Spice</p>
                                        <div className="grid grid-cols-4 gap-1.5">
                                            {["", "1", "2", "3"].map((n) => (
                                                <button
                                                    key={n || "0"}
                                ...
                                {showPhysical ? (
                                    <FieldLabel label="SKU (optional)">
                                        <Input value={sku} onChange={(e) => setSku(e.target.value)} placeholder="Internal code" className="h-11 rounded-2xl" />
                                    </FieldLabel>
                                ) : null}
                                <FieldLabel label="Short description">
                                    <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="What is this item?" rows={3} className="rounded-2xl" />
                                </FieldLabel>
                                {showPhysical && fulfillment !== "DIGITAL" ? (
                                    <>
                                        <FieldLabel label="Variants (one per line)">
                                            <Textarea value={variantsText} onChange={(e) => setVariantsText(e.target.value)} placeholder={"S\nM\nL"} rows={3} className="rounded-2xl" />
                                        </FieldLabel>
                                        <p className="px-0.5 text-[11px] font-medium text-muted-foreground">How customers get it</p>
                                        ...
                                        {shipMode === "DELIVER" || shipMode === "BOTH" ? (
                                            <FieldLabel label="Delivery fee">
                                                <Input type="number" min="0" step="0.01" value={shipFee} onChange={(e) => setShipFee(e.target.value)} placeholder="0" className="h-11 rounded-2xl" />
                                            </FieldLabel>
                                        ) : null}
                                ...
                                <FieldLabel label="Compare-at price (optional)">
                                    <Input type="number" min="0" step="0.01" value={compareAt} onChange={(e) => setCompareAt(e.target.value)} placeholder="Shown as the old price" className="h-11 rounded-2xl" />
                                </FieldLabel>
                            </div>
                        ) : null}
                    </div>
