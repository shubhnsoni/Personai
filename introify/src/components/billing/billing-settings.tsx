...
export function BillingSettings({ data, initialCadence, checkoutReturn }: { data: BillingDashboard; initialCadence?: BillingCadence; checkoutReturn?: "success" | "cancelled" }) {
    ...
    return <main className="billing-design billing-dashboard">
        ...
        {!data.availability.billing && <div className="billing-note plan-launch-note"><Info size={17} aria-hidden="true" /><div><p><strong>Checkout is not open yet.</strong></p><p>Paid subscriptions and one-time packs cannot be purchased. Your current Free plan stays in place. Join the waitlist from the pricing page, or email support from Contact.</p></div></div>}
