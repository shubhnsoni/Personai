...
/**
 * Guest-honest empty catalog for REAL_ESTATE_BROKERAGE COLLECT_LEADS kits.
 * Same max-w-5xl catalog shell as EventsGuestMenu - never Shop badge or products wording.
 */
export function RealestateGuestMenu({
    slug,
    name,
    logoUrl,
    whatsapp,
    ...
    const label = realestateGuestMenuLabel(role)
    const empty = realestateGuestMenuEmptyCopy({ displayName: name, role, primaryGoal })
    const subtitle =
        label === REALESTATE_GUEST_MENU_LABEL_LISTINGS
            ? "Selected listings and conversations - enquire on the home page."
            : "Selected listings and conversations - enquire on the home page."

    return (
        <div data-realestate-guest-menu="true" className="min-h-dvh bg-background text-foreground">
            <CatalogHeader
                slug={slug}
                name={name}
                logoUrl={logoUrl}
            ...
            <main className="mx-auto max-w-5xl space-y-6 px-3 py-5 pb-16 lg:px-4 lg:py-6">
                <section className="rounded-2xl border border-border bg-card px-4 py-4 lg:px-5">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                        <Link
                            ...
                            data-realestate-menu-chat="true"
                        >
                            {empty.chatLabel}
                        </Link>
                    </div>
                ...
                <section
                    data-realestate-menu-empty="true"
