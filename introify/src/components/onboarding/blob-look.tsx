...
export function BlobLookStudio({
    ...
    return (
        <div className="mt-4 space-y-8">
            ...
            <section className="space-y-3">
                <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-white/35">More bots</p>
...
function LivePagePreview({ name, look }: { name: string; look: BloubPick }) {
    ...
    return (
        <div
            ...
            style={{ background: `radial-gradient(circle at 50% 28%, ${colors[0]}22, #09090b 62%)` }}
        >
            <p className="px-5 pt-4 text-[11px] uppercase tracking-[0.18em] text-white/35">Live page</p>
            <div className="flex min-h-[22rem] flex-col items-center justify-center px-6 pb-8 pt-6 text-center">
                <p className="text-sm text-white/55">Hi!</p>
                <h3 className="mt-2 text-2xl font-medium tracking-tight text-white">{name}'s AI</h3>
                <div className="mt-8">
                    <WelcomeOrb
                        size={168}
                        look="bloub"
                ...
                <p className="mt-8 rounded-full bg-white/10 px-4 py-2 text-[13px] text-white/70">What should I know about {name}?</p>
            </div>
        </div>
    )
}
