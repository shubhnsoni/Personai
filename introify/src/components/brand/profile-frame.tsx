...
export function ProfileFrame({ className, href = "/demo" }: ProfileFrameProps) {
    return (
        <Link
            href={href}
            aria-label="Open the live Introify demo profile"
            className={cn(
        ...
        >
            <figure className="overflow-hidden rounded-2xl border border-white/10 bg-profile shadow-[0_40px_80px_-32px_rgba(0,215,255,0.28)] transition-transform duration-300 group-hover:-translate-y-0.5">
                <div className="flex items-center gap-2 border-b border-white/5 bg-black/40 px-4 py-2.5">
                    ...
                    <span className="rounded-full border border-cyan-400/30 bg-cyan-400/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-cyan-300">
                        Live
                    </span>
                </div>
                <Image
                    src="/demo-profile.jpg"
                    alt="Riley Vale's live Introify profile — chat, book a call, and buy"
                    width={1280}
                    height={720}
                    priority
