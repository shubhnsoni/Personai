...
| `src/lib/business-os/{engines,blueprints,types,validation,workflow}.ts` | `personai-business-os-consolidation-wt/introify` | the three existing blueprints and the real capability ids |
| `prisma/schema.prisma`, `src/lib/restaurant-order*.ts`, `src/lib/commerce.ts`, `src/lib/menu.ts` | `personai-business-os-consolidation-wt/introify` | what is actually implemented versus declared |
