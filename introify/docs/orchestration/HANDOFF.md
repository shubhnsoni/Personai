# PersonaAI / Introify — Session Handoff
...
| | |
|---|---|
| Repo | `C:\Users\shubh\Desktop\Projects\personal projects\personai` (app in `introify/`) |
...
to refuse `personalink`, `Introify` and `PERSONALINK`, and to refuse even the other disposable
...
Run from `introify/`. Repo-wide `npm run lint` exits 1 with ~124 problems at baseline; that is an
