...
### The reproducible gate is the load-bearing change
    ...
    cd introify && node scripts/gates/run-gates.js
...
| 9 | **Worktree portability** | 10 absolute paths are baked into `latest.json`; untracked `introify/.env` is required to run anything. Cosmetic for correctness, real for reproducibility claims. |
...
- **New, from the S0 environment work:** any isolated verification worktree needs `introify/.env` as a
