...
describe("ProfileView - intro chips follow contentDisplayMode", () => {
    ...
    it("puts WhatsApp, maps and Zomato on a smaller row under the chips, not among them", () => {
        ...
        const ask = document.querySelector("[data-ask-about]")
        expect(chips?.contains(contact)).toBe(false)
        expect(document.querySelector("[data-chat-header-links]")).toBeNull()
        expect(ask?.contains(contact) || Boolean(contact)).toBe(true)
