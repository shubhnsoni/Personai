...
import { extraDetailPhotos, parsePdpDisplay, writePdpDisplay } from "@/lib/shop/pdp-display"
...
describe("pdp content", () => {
    ...
    it("uses extra gallery shots as the More details stack, never duplicates", () => {
        expect(pdpMoreImages(["/cover.jpg", "/a.jpg", "/b.jpg", "/a.jpg"])).toEqual(["/a.jpg", "/b.jpg"])
        expect(pdpMoreImages(["/cover.jpg"])).toEqual(["/cover.jpg"])
        expect(pdpMoreImages([], "/life.jpg")).toEqual(["/life.jpg"])
        expect(pdpMoreImages([])).toEqual([PDP_LIFESTYLE])
    })

    it("only treats extra gallery shots as more-details photos", () => {
        expect(extraDetailPhotos(["/cover.jpg", "/a.jpg", "/b.jpg", "/a.jpg"])).toEqual(["/a.jpg", "/b.jpg"])
        expect(extraDetailPhotos(["/cover.jpg"])).toEqual([])
        expect(extraDetailPhotos([])).toEqual([])
    })

    it("defaults product-page modules off until the owner turns them on", () => {
        expect(parsePdpDisplay(null)).toEqual({
            showMore: false,
            showRelated: false,
            showBestsellers: false,
        })
        const json = writePdpDisplay(null, { showMore: true, showRelated: true, showBestsellers: false })
        expect(parsePdpDisplay(json)).toEqual({
            showMore: true,
            showRelated: true,
            showBestsellers: false,
        })
