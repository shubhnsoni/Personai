 ...
 * environment configuration (introify/.env, or DATABASE_URL already exported).
 *
    ...
    if (!fs.existsSync(p)) {
      throw new Error(
        `Layout assertion failed: expected ${p} to exist. ` +
          ...
          "if that is wrong, the file has been moved out of introify/scripts/gates/.",
      );
    }
  }
}
...
function usage() {
  return [
    "Usage: node scripts/gates/run-gates.js [options]   (run from introify/)",
    "",
    ...
    "  --manifest=<path>      alternate manifest (relative to introify/)",
    "  --out-dir=<path>       alternate artefact root (relative to introify/)",
    "",
    "Environment:",
...
function buildCommand(runner, harnessRelPath) {
  if (runner === "ts-node-checks") {
    ...
    if (!fs.existsSync(tsNodeBin)) {
      throw new Error(
        `ts-node is not installed at ${path.relative(APP_DIR, tsNodeBin)}. Run npm install in introify/ first.`,
      );
    }
    return {
...
async function main() {
  ...
  if (!opts.list && !opts.integrityOnly && !executionBlocked) {
    ...
    if (dbTarget) {
      ...
      //    database would silently hand every harness whatever introify/.env points at. So the
