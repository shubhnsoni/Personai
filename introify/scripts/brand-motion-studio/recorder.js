...
export function createRecorder(hooks){
 ...
 function take(){return validateTake({name:$('take-name').value,duration:state.duration,loop:state.loop,keyframes:frames,rangeStart:hooks.getConfig().rangeStart,rangeEnd:hooks.getConfig().rangeEnd});}
 ...
 function capture(automatic=false,position=hooks.getProgress()){
   let time=Math.round(position*1000)/1000;
 ...
 function finishRecord(){state.recording=false;hooks.pause();if(frames.length){hooks.setProgress(1);if(state.loop)hooks.setConfig({...frames[0].settings,rangeStart:hooks.getConfig().rangeStart,rangeEnd:hooks.getConfig().rangeEnd});capture(false,1);}state.preview=frames.length>=2;cached=null;hooks.refresh();render();hooks.status('Recording stopped. Review the keyframes, then save this as a new take.');}
 $('add-key').onclick=()=>{capture();hooks.status('Keyframe saved at the current timeline position.');};
 $('record-key').onclick=()=>{if(state.recording)return finishRecord();state.preview=false;state.recording=true;lastAuto=-1;hooks.setProgress(0);capture();hooks.refresh();hooks.play();render();hooks.status('Recording one pass. Move any control, or use Add keyframe, to capture changes.');};
 ...
 function load(input){const t=validateTake(input);frames=t.keyframes;state.duration=t.duration;state.loop=t.loop;state.preview=false;state.recording=false;selected=null;$('take-name').value=t.name;$('take-duration').value=t.duration;$('take-loop').checked=t.loop;hooks.pause();hooks.setProgress(0);hooks.setConfig({...frames[0].settings,rangeStart:t.rangeStart,rangeEnd:t.rangeEnd});persist();}
