...
async function main() {
    ...
    try {
        ...
        check("the timeline is 200 and ordered", events.status === 200 && seqs.length > 0 && seqs.every((v, i) => i === 0 || v > seqs[i - 1]), `n=${seqs.length}`)
        check("timeline seq serialises as a string not a BigInt", rows.length > 0 && rows.every((e) => typeof pick(e, "seq") === "string"), typeof pick(rows[0], "seq"))
