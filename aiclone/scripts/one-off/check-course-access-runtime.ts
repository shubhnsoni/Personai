...
async function main() {
    ...
    try {
        ...
        checkInvertible("REVOKED is terminal, so a revoked entitlement cannot be revived", accessGrantFlow.isTerminal("REVOKED"))
        checkInvertible("SUSPENDED is reversible, because suspension is not revocation", accessGrantFlow.can("SUSPENDED", "ACTIVE"))
        checkInvertible("EXPIRED can be re-activated, because a renewal is a normal thing", accessGrantFlow.can("EXPIRED", "ACTIVE"))
        checkInvertible(
        ...
        checkInvertible("anonymous tier definition refused UNAUTHORIZED", !anonDefine.ok && anonDefine.code === "UNAUTHORIZED", why(anonDefine))
        checkInvertible("anonymous tier listing refused UNAUTHORIZED", !anonList.ok && anonList.code === "UNAUTHORIZED", why(anonList))
        checkInvertible("anonymous wrote zero tiers", (await prisma.courseAccessLevel.count()) === base.levels)
        ...
        checkInvertible("rank 0 is refused, because rank is what orders the tiers", !zeroRank.ok && zeroRank.code === "CONFLICT", why(zeroRank))
        const negPrice = await attempt(() =>
            access.defineLevel(ids.wsA, { courseId: ids.courseA, key: "neg", label: "Neg", rank: 9, priceCents: -1 }, actor),
        )
        checkInvertible("a negative price is refused", !negPrice.ok && negPrice.code === "CONFLICT", why(negPrice))
        ...
        checkInvertible("tiers are created and ordered by rank", basic.rank === 1 && pro.rank === 2)
        const dupRank = await attempt(() =>
            access.defineLevel(ids.wsA, { courseId: ids.courseA, key: "other", label: "Other", rank: 1 }, actor),
        )
        checkInvertible("a duplicate rank on one course is a CONFLICT", !dupRank.ok && dupRank.code === "CONFLICT", why(dupRank))
        ...
        checkInvertible("three lessons carry a rule and one deliberately does not", rules.length === 3, `rules=${rules.length}`)
        ...
        checkInvertible("a rule on a nonexistent lesson is FORBIDDEN", !foreignLesson.ok && foreignLesson.code === "FORBIDDEN", why(foreignLesson))
        ...
        checkInvertible("a new grant starts PENDING, not ACTIVE", grantBasic.grant.state === "PENDING", `state=${grantBasic.grant.state}`)
        checkInvertible("a PENDING grant entitles nothing", grantBasic.grant.entitles === false)
        ...
        checkInvertible("a basic holder sees exactly the unrestricted lesson and the basic lesson", basicView.visibleCount === 2, `visible=${basicView.visibleCount}`)
        checkInvertible("and the two pro lessons are locked, not hidden", basicView.lockedCount === 2, `locked=${basicView.lockedCount}`)
        checkInvertible("the held tier is reported back", basicView.heldLevelKey === "basic" && basicView.heldRank === 1)
        checkInvertible(
        ...
        checkInvertible("a pro holder sees all four lessons", proView.visibleCount === 4 && proView.lockedCount === 0, `visible=${proView.visibleCount}`)
        ...
        checkInvertible("and is told plainly that the enrolment holds no tier", /holds no tier/.test(noneView.lessons[1].reason), noneView.lessons[1].reason)

        const perLesson = await learner.canViewLesson({ lessonId: lessons[1], memberId: ids.memberBasic })
        checkInvertible("the per-lesson gate agrees with the list for a permitted lesson", perLesson.allowed)
        const perLessonDenied = await learner.canViewLesson({ lessonId: lessons[2], memberId: ids.memberBasic })
        checkInvertible("the per-lesson gate agrees with the list for a locked lesson", !perLessonDenied.allowed, perLessonDenied.reason)
        ...
        checkInvertible("the report still names the suspension so the surface can explain it", suspendedView.grantState === "SUSPENDED")
        await access.transitionGrant(ids.wsA, ids.courseA, grantPro.grant.id, "ACTIVE", actor)
        const restored = await learner.visibleLessons({ courseId: ids.courseA, memberId: ids.memberPro })
        checkInvertible("reinstating restores the full view", restored.visibleCount === 4)
        ...
        checkInvertible("requesting a move to the tier already held is refused", !sameTier.ok && sameTier.code === "CONFLICT", why(sameTier))
        ...
        checkInvertible("the direction is derived from rank, not taken from the caller", change.change.direction === "UPGRADE", `direction=${change.change.direction}`)
        ...
        checkInvertible("replaying the change key returns the original", changeReplay.replayed && changeReplay.change.id === change.change.id)
        ...
        checkInvertible("requesting an upgrade changes nothing about what the learner can see", stillBasic.visibleCount === 2, `visible=${stillBasic.visibleCount}`)
        ...
        checkInvertible("applying moves the entitlement", applied.grant.accessLevelKey === "pro", `held=${applied.grant.accessLevelKey}`)
        checkInvertible("and the change becomes APPLIED and terminal", applied.change.state === "APPLIED" && applied.change.allowedTransitions.length === 0)
        const afterApply = await learner.visibleLessons({ courseId: ids.courseA, memberId: ids.memberBasic })
        checkInvertible("the learner now sees everything", afterApply.visibleCount === 4, `visible=${afterApply.visibleCount}`)
        ...
        checkInvertible("an applied change cannot be applied twice", !reapply.ok && reapply.code === "CONFLICT", why(reapply))
        ...
        checkInvertible("a move to a lower rank is derived as a DOWNGRADE", down.change.direction === "DOWNGRADE")
        ...
        checkInvertible("once the tier matches again the downgrade applies", downApplied.grant.accessLevelKey === "basic")
        ...
        checkInvertible("an unknown course is refused", !unknownCourse.ok && unknownCourse.code === "FORBIDDEN", why(unknownCourse))
        checkInvertible(
        ...
        checkInvertible("a request with no learner identity is refused", !noIdentity.ok && noIdentity.code === "FORBIDDEN", why(noIdentity))
        const byEmail = await learner.visibleLessons({
            courseId: ids.courseA,
            memberEmail: `${ids.memberPro}@example.test`,
        })
        checkInvertible("a learner can be resolved by email as well as by member id, matching the existing library reader", byEmail.visibleCount === 4)
        ...
        checkInvertible("another owner cannot list this course's tiers", !foreignLevels.ok && foreignLevels.code === "FORBIDDEN", why(foreignLevels))
        checkInvertible(
        ...
        checkInvertible("the database refuses to rewrite the access history", !rewrite.ok, why(rewrite))
        ...
        checkInvertible("zero external calls were made by the access runtime", fetchCalls === 0, `fetchCalls=${fetchCalls}`)
    } finally {
